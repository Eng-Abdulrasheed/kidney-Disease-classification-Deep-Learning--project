...
def receive_video_frames():
    if 'video_frames' not in request.files:
        return jsonify({'error': 'No video frames found'})

    video_frame = request.files['video_frames'].read()
    nparr = np.frombuffer(video_frame, np.uint8)
    frame = cv2.imdecode(nparr, cv2.IMREAD_COLOR)

    # Save the received frame as an image file
    frame_path = os.path.join(app.config['UPLOAD_FOLDER'], f'frame_{uuid.uuid4()}.jpg')
    cv2.imwrite(frame_path, frame)

    return jsonify({'message': 'Video frame received and saved'})
